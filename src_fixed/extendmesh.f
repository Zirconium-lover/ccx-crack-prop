...
!     
     ...
     &           neigh,kneigh)
            if(neigh(1).lt.1) neigh(1)=1
            if(neigh(1).gt.k) neigh(1)=k
            neighbor(j)=istartfront(i)-1+neigh(1)
         enddo    
         neighbor(m)=iendfront(i)
c        --- sanitize neighbor indices (bounds) ---
         do j=l,m
            if(neighbor(j).lt.istartfront(i)) neighbor(j)=istartfront(i)
            if(neighbor(j).gt.iendfront(i))   neighbor(j)=iendfront(i)
         enddo

c        --- enforce monotonicity + limit jump (forward) ---
         do j=l+1,m
            if(neighbor(j).lt.neighbor(j-1)) neighbor(j)=neighbor(j-1)
            if(neighbor(j).gt.neighbor(j-1)+maxjump)
     &           neighbor(j)=neighbor(j-1)+maxjump
         enddo

c        --- keep the end anchored, then smooth backward ---
         neighbor(m)=iendfront(i)
         do j=m-1,l,-1
            if(neighbor(j).gt.neighbor(j+1)) neighbor(j)=neighbor(j+1)
            if(neighbor(j).lt.neighbor(j+1)-maxjump)
     &           neighbor(j)=neighbor(j+1)-maxjump
         enddo
         neighbor(l)=istartfront(i)
!     
