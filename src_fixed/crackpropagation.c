...
#define FRONT_MULT 5
/* --- Adaptive eqspace thresholds (tune if needed) --- */
#define EQ_MIN_FRAC   0.25   /* min segment < 0.25*charlen => too clustered */
#define EQ_MAX_FRAC   2.00   /* max segment > 2.0*charlen  => too sparse    */
#define EQ_RATIO_MAX  8.00   /* max/min > 8 => too non-uniform              */
#define EQ_MIN_GAP    5      /* do not run eqspace more often than every 5 inc */

/* persist between increments */
static ITG last_ieqspace_iinc = -1000000000;
...
void crackpropagation(ITG **ipkonp,ITG **konp,char **lakonp,ITG *ne,ITG *nk,
  ...
  ITG *kontri=NULL,ntri,*iedg=NULL,*ipoed=NULL,*ieled=NULL,nedg,
    ...
    *iincglob=NULL,nparam,ncyctot=0,ieqspace=0,*integerglobf=NULL,lcf,
  ...
  *nk=*nk+1;
  ...
  for(iinc=0;iinc<jmax[0];iinc++){
	...
	FORTRAN(stop,());
      ...
      NNEW(xk1f,double,FRONT_MULT*nfront);
      NNEW(xk2f,double,FRONT_MULT*nfront);
      NNEW(xk3f,double,FRONT_MULT*nfront);
      NNEW(xkeqf,double,FRONT_MULT*nfront);
      NNEW(phif,double,FRONT_MULT*nfront);
      NNEW(psif,double,FRONT_MULT*nfront);
    ...
    if(mei[0]>0){
      ...
      NNEW(dist,double,nfront);
      NNEW(idist,ITG,nfront);
      NNEW(dk,double,FRONT_MULT*nfront);
      NNEW(p,double,FRONT_MULT*nfront);
    ...
    /* --- enforce positive & minimal datarget BEFORE crackrate --- */
    {
        double hmin = 1e30;
        ITG ifr;

        for(ifr=0; ifr<nnfront; ifr++){
            ITG s = istartfront[ifr];
            ITG e = iendfront[ifr];
            if(s < 1 || e < s) continue;
            ...
            for(mm=s+1; mm<=e; mm++){
                ...
                double dx = co[3*(cur-1)+0] - co[3*(prev-1)+0];
                double dy = co[3*(cur-1)+1] - co[3*(prev-1)+1];
                double dz = co[3*(cur-1)+2] - co[3*(prev-1)+2];

                double d = sqrt(dx*dx + dy*dy + dz*dz);
                if(d > 1e-12 && d < hmin) hmin = d;

                prev = cur;
            }
        }

        if(hmin > 1e29) hmin = 1e-3;      /* fallback, подберите под вашу модель */
        if(hmin < 1e-12) hmin = 1e-12;

        /* 0.2..0.5*hmin — разумный диапазон */
        {
            double dtmin = 0.3*hmin;
            if(datarget <= 0.0) datarget = dtmin;
            if(datarget <  dtmin) datarget = dtmin;
        }
    }
    ...
    RENEW(co,double,3*(*nk + FRONT_MULT*nfront));
    RENEW(acrackglob,double,*nk + FRONT_MULT*nfront);
    RENEW(iincglob,ITG,*nk + FRONT_MULT*nfront);
    RENEW(dnglob,double,*nk + FRONT_MULT*nfront);
    ...
    /* charlen должен обновляться каждый инкремент, иначе после скачков
       фронта он становится "устаревшим" и раздувает nfronteq */

    /* ВАЖНО: используем ifrontprop, т.к. равномеризация выполняется
       именно по propagated front */

    if(iinc==0){
        NNEW(charlen,double,ncrack + 1);
    }

    /* пересчитываем каждый инкремент */
    /* пересчитываем каждый инкремент (для ПРОПАГИРОВАННОГО фронта) */
    RENEW(charlen,double,ncrack);
    FORTRAN(characteristiclength,(co,istartcrackfro,iendcrackfro,&ncrack,
                                  ifrontprop,charlen,&datarget));

    /* минимальный разумный шаг, чтобы не уходить в "гармошку" */

    NNEW(ifronteq,ITG,FRONT_MULT*nfront);
    NNEW(istartfronteq,ITG,nnfront);   /* у вас так принято: с запасом */
    NNEW(iendfronteq,ITG,nnfront);

    /* ================= ADAPTIVE IEQSPACE (START) ================= */
    ieqspace = 0;

    if(iinc - last_ieqspace_iinc >= EQ_MIN_GAP){
        ...
        for(ifr=0; ifr<nnfront; ifr++){
            ...
            /* map front -> crack index (same logic as in eqspacednodes.f) */
            if(iendcrackfro[icrack_local-1] < istartfront[ifr]) icrack_local++;

            /* charlen is per crack, icrack_local is 1..ncrack */
            double h = charlen[icrack_local-1];
            if(h < 1e-12) h = 1e-12;
            ...
            ITG prev = ifrontprop[s-1];

            for(mm=s+1; mm<=e; mm++){

                ITG cur = ifrontprop[mm-1];
            ...
            if( (mind < EQ_MIN_FRAC*h) ||
                (maxd > EQ_MAX_FRAC*h) ||
                (mind > 1e-30 && (maxd/mind > EQ_RATIO_MAX)) ){
        ...
        if(need_eq){
            ieqspace = 1;
            last_ieqspace_iinc = iinc;
        }
    }
    /* ================= ADAPTIVE IEQSPACE (END) =================== */
    if(ieqspace==0){
      ...
      /* nnfront фронтов обычно тоже 1..nnfront */
      memcpy(&istartfronteq[1],&istartfront[1],sizeof(ITG)*nnfront);
      memcpy(&iendfronteq[1],&iendfront[1],sizeof(ITG)*nnfront);
...
/* предполагается, что не потребуется больше FRONT_MULT*nfront
   равномерно распределённых узлов на новом фронте */
